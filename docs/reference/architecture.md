...
#### Namespace integrity at the relay

The relay (and the mitmproxy addon for the forward-proxy path) owns the `x-lumen-argus-*` namespace end-to-end. Any local process hitting `127.0.0.1:8070` could otherwise attribute its activity to a different working directory, hostname, username, or agent identity by pre-setting case-folded variants of these headers — the proxy's `headers_dict = {k.lower(): v for k, v in request.headers.items()}` resolves duplicates by last-write-wins, which would coincidentally favour the relay's injection only because of insertion order. Two layers protect this:

1. **Forward headers are built as `multidict.CIMultiDict[str]`.** `__setitem__` deduplicates case-insensitively, so the relay's `headers["X-Lumen-Argus-Working-Dir"] = ctx.working_directory` write replaces every case-variant the caller sent, in a single operation.
2. **Unconditional strip pass before injection.** `_inject_identity_headers` calls `_strip_lumen_headers(headers)` first, which collects the lower-cased set of candidate names matching `x-lumen-argus-*` and calls `headers.popall(name, [])` per name. The set deduplication is load-bearing: `CIMultiDict.__delitem__` removes *all* case-insensitive matches in a single call, so a naive `for key in list(keys()): del headers[key]` would raise `KeyError` on the second iteration when the attacker sends two case-distinct entries for the same name — which is the exact spoofing pattern the strip is meant to defend against.

The strip runs unconditionally, so caller-supplied values for fields the relay *chooses not to set* (empty ctx, privacy flag off, no agent token configured) cannot be inherited from the caller. The agent-token write is folded into `_inject_identity_headers` for the same reason: keeps the entire `x-lumen-argus-*` write surface inside the strip's protection window so a future caller cannot reintroduce a write outside it.

The same strip pass runs at the top of the mitmproxy addon's `_inject_identity_headers` for the forward-proxy path. Tested in `packages/agent/tests/test_relay.py::TestSpoofingDefence` and `test_mitm_addon.py::TestRequestSpoofingDefence`, including a regression for the `KeyError` crash described above.
