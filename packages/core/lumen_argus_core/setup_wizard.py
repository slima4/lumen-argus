...
def _setup_forward_proxy(
    ...
    """Set up forward proxy for a tool that requires TLS interception.

    Steps:
    1. Generate CA cert (if not exists)
    2. Write aliases file
    3. Add source line to shell profile
    4. Optionally install CA to system trust store
    """
    ...
    if not non_interactive:
        if not _prompt_yes("  Step 1/4: Set up forward proxy for %s?" % target.display_name):
            print("  Skipped.")
            return changes
    ...
    # Use $HOME-relative path in alias to avoid hardcoded paths and shell injection
    alias_content = (
        "# lumen-argus forward proxy aliases\n"
        "# Auto-generated by lumen-argus setup — do not edit manually\n\n"
    ...
    # Step 3: Add source line to shell profile
    if profile_path:
        if not non_interactive:
            print()
            add_source = _prompt_yes("  Step 3/4: Add source line to %s?" % profile_path)
        else:
            add_source = True
