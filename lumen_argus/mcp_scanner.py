"""Shared MCP scanning logic — used by both HTTP proxy and mcp-wrap.
...
Used by:
- proxy.py — MCP-aware scanning of HTTP MCP traffic (enterprise/k8s)
- mcp_wrap.py — stdio MCP wrapper (local development)
"""
...
class MCPScanner:
    ...
    Used by both the HTTP proxy (for MCP over HTTP) and mcp-wrap (for stdio).
